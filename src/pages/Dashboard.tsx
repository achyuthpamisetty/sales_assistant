...
import {
  Users, Briefcase, PhoneCall, TrendingUp, ArrowRight, DollarSign, Flame,
  Calendar, Mail, HelpCircle, BarChart2
} from 'lucide-react';

const Dashboard = () => {
  const { leads, accounts, contacts, opportunities, loading } = useSalesforce();
  const userName = "John Doe"; // Replace with actual user
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <StatCard title="Total Revenue" value={`$${totalRevenue.toLocaleString()}`} change={15.8} icon={DollarSign} color="bg-green-600" />
        <StatCard title="Leads" value={leads.length} change={5.2} icon={Users} color="bg-blue-600" />
        <StatCard title="Accounts" value={accounts.length} change={2.3} icon={Briefcase} color="bg-indigo-600" />
        <StatCard title="Contacts" value={contacts.length} change={-1.5} icon={PhoneCall} color="bg-violet-600" />
        <StatCard title="Open Opportunities" value={`$${(1750000).toLocaleString()}`} change={12.5} icon={TrendingUp} color="bg-teal-600" />
      </div>
      ...
      {/* Recent Communications + New Email Button */}
      <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2">
            <Mail size={18} /> Recent Communications
          </h2>
          <Link
            to="/email/new"
